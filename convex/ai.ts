...
export const generateAiSubmissions = internalAction({
  ...
  handler: async (ctx, { gameId, roundId }) => {
    const roundInfo = await ctx.runQuery(internal.aiQueries.getRoundInfo, {
      roundId,
      gameId,
    });
    ...
    for (const aiPlayer of aiPlayersToSubmit) {
      ...
      const cached = await ctx.runQuery(internal.aiQueries.getCachedResponse, {
      ...
      if (cached && cached.responses.length > 0) {
        ...
        await ctx.runMutation(internal.aiQueries.saveToCache, {
          promptText: promptCard.text,
          personaId: aiPlayer.aiPersonaId,
      ...
      await ctx.runMutation(internal.aiQueries.submitAiCard, {
        roundId,
    ...
    await ctx.runMutation(internal.aiQueries.checkAndMoveToJudging, {
      roundId,
      gameId,
    });
  },
